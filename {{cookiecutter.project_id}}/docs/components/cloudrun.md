...
- You can also list all deployed Cloud Run services:
  ```
  gcloud run services list --region={{cookiecutter.gcp_region}} --format="value(name,status.url)"
  ```
...
Forwarding service projects/{{cookiecutter.project_id}}/locations/{{cookiecutter.gcp_region}}/services/frontend-angular to local port 9001
