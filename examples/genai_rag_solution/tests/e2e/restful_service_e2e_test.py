...
def service_url():
  service_url = exec_output(
      "gcloud run services describe restful-service --region=us-central1 --format='value(status.url)'"
  )
...
def test_restful_create_get(auth_token, service_url):
  ...
  # Create a new item
  url = service_url + f"/restful-service/todo"
  res = requests.post(url, json=data, headers=headers)
  assert res.status_code == 200

  # Get the item by ID
  url = service_url + f"/restful-service/todo/{test_id}"
